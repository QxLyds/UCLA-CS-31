...
void Player::move(int dir)
{
    m_age++;
        ...
        {
            case UP:
                if (m_row > 2 && m_pit->numberOfSpidersAt(m_row - 1, m_col) > 0)
                {
                    m_pit->destroyOneSpider(m_row - 1, m_col);
                    m_row -=2;
                    
                }else if (m_row  - 1 == 1 && m_pit->numberOfSpidersAt(m_row - 1, m_col) > 0 )
                {
                    m_row--;
                    m_pit->destroyOneSpider(m_row, m_col);
                    
                }else if (m_row > 1 && m_pit->numberOfSpidersAt(m_row - 1, m_col) == 0)
                    m_row--;
                break;
                
            case DOWN:
                if (m_row < m_pit->rows() - 1 && m_pit->numberOfSpidersAt(m_row + 1, m_col) > 0)
                {
                    m_pit->destroyOneSpider(m_row + 1, m_col);
                    m_row +=2;
                    
                }else if (m_row + 1 == m_pit->rows() && m_pit->numberOfSpidersAt(m_row + 1, m_col) > 0 )
                {
                    m_row++;
                    m_pit->destroyOneSpider(m_row, m_col);
                    
                }else if (m_row < m_pit->rows() && m_pit->numberOfSpidersAt(m_row + 1, m_col) == 0)
                    m_row++;
                break;
                
            case LEFT:
                if (m_col > 2 && m_pit->numberOfSpidersAt(m_row, m_col - 1) > 0 )
                {
                    m_pit->destroyOneSpider(m_row, m_col - 1);
                    m_col -=2;
                    
                }else if (m_col - 1 == 1 && m_pit->numberOfSpidersAt(m_row, m_col - 1) > 0 )
                {
                    m_col--;
                    m_pit->destroyOneSpider(m_row, m_col);
                    
                }else if (m_col > 1 && m_pit->numberOfSpidersAt(m_row, m_col - 1) == 0)
                    m_col--;
                break;
                
            case RIGHT:
                if (m_col < m_pit->cols() - 1 && m_pit->numberOfSpidersAt(m_row, m_col + 1) > 0)
                {
                    m_pit->destroyOneSpider(m_row, m_col + 1);
                    m_col +=2;
                    
                }else if (m_col + 1 == m_pit->cols() && m_pit->numberOfSpidersAt(m_row, m_col + 1) > 0 )
                {
                    m_col++;
                    m_pit->destroyOneSpider(m_row, m_col);
                    
                }else if (m_col < m_pit->cols() && m_pit->numberOfSpidersAt(m_row, m_col + 1) == 0)
                    m_col++;
                break;
            default:
                break;
...
int Pit::spiderCount() const // number of total spiders
{
    return m_nSpiders;
}
           ...
           for (c = 0; c < cols(); c++)
           {
               if (numberOfSpidersAt(r + 1, c + 1) == 1)
                   grid[r][c] = 'S';
                    ...
                    grid[r][c] = '9';
               else if (numberOfSpidersAt(r + 1, c + 1) > 1 && numberOfSpidersAt(r + 1, c + 1) < 9)
...
bool Pit::addSpider(int r, int c)
{
    ...
    if (spiderCount() == MAXSPIDERS )
      ...
      // Otherwise, dynamically allocate a new spider at coordinates (r,c).
        m_spiders[spiderCount()] = new Spider(this, r, c);
        m_nSpiders++;
            return true;
}
...
{
    //k-th spider in the pit make one move
    ...
    if (numberOfSpidersAt(m_player->row(), m_player->col()) > 0)
        m_player->setDead();
